...
class PGAgent(BaseAgent):
    ...
    def train(self, observations, actions, rewards_list, next_observations, terminals):
        ...
        if not self.use_gae:
            ...
            advantages_hat = np.zeros_like(rewards_concat)
            if self.nn_baseline:
                baselines_normalized = self.actor.run_baseline_prediction(observations)
                baselines = baselines_normalized * np.std(q_values) + np.mean(q_values)
            else:
                baselines = np.zeros_like(rewards_concat)
            for t in reversed(range(rewards_concat.shape[0])):
                if terminals[t]:
                    delta = rewards_concat[t] - baselines[t]
                    advantages_hat[t] = delta
                else:
                    # delta = reward + gamma * next_baseline - baseline
                    # advantage_hat = delta + gamma * lambda * next_advantage_hat
                    delta = rewards_concat[t] + self.gamma * baselines[t + 1] - baselines[t]
                    advantages_hat[t] = delta + self.gamma * self.lam * advantages_hat[t + 1]
            advantages = advantages_hat
    ...
    def _discounted_return(self, rewards):
        ...
        list_of_discounted_returns = sum(rewards * discounts) * np.ones_like(rewards)
    ...
    def _discounted_cumsum(self, rewards):
        ...
        # for loop takes less number of operations than vectorized solution
        for start_time_index in range(len(rewards)):
            ...
            list_of_discounted_cumsums.append(sum_discounted_rtg)
