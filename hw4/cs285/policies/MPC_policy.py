...
class MPCPolicy(BasePolicy):
    ...
    def calculate_sum_of_rewards(self, obs, candidate_action_sequences, model):
        ...
        done_flags = np.zeros(self.N, dtype=int)
        obs_batch = np.tile(obs, (self.N, 1))

        for t in range(self.horizon):
            acs_batch = candidate_action_sequences[:, t]
            reward_batch, done_batch = self.env.get_reward(obs_batch, candidate_action_sequences)
            obs_batch = model.get_prediction(obs_batch, acs_batch, self.data_statistics)

            sum_of_rewards += reward_batch * (1-done_flags)
            done_flags = np.maximum(done_flags, done_batch)
