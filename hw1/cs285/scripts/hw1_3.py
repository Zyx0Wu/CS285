...
def main(num_steps=1000):
    ...
    args.num_agent_train_steps_per_iter = num_steps
    params = vars(args)
...
if __name__ == "__main__":
    steps = list(range(300,3001,300))
    eval_means, eval_stds = [], []
    for i in steps:
        eval_mean, eval_std = main(i)
        eval_means.append(eval_mean)
        eval_stds.append(eval_std)

    expert_means = [4713.65] * len(steps)
    expert_stds = [0.00] * len(steps)

    plt.figure()
    plt.errorbar(steps, eval_means, eval_stds, marker='o', capsize=8, linestyle='--', label='Behavior Cloning')
    plt.errorbar(steps, expert_means, expert_stds, marker='o', capsize=8, linestyle='--', label='Expert')
    ...
    plt.savefig(save_dir / 'bc1_3_1.png', bbox_inches='tight', pad_inches=0.2, dpi=300)
    plt.show()
